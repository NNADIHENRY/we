package com.HITech.HILearn.adapter;
...
public class SubAdapter extends RecyclerView.Adapter<SubAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull SubAdapter.ViewHolder holder, int position) {
        ...
        holder.cell.setBackgroundResource(drawableList.get(position).drawable);
        Drawable drawable;
        if (main_id == 4) {
            drawable = context.getDrawable(Constant.getMixedIcons().get(position));

        } else if (main_id == 1) {
            drawable = context.getDrawable(Constant.getFirstDefaultIcons1().get(position));

        }else if (main_id == 2) {
            drawable = context.getDrawable(Constant.getSecondDefaultIcons1().get(position));

        }
        else {

            drawable = context.getDrawable(Constant.getThirdDefaultIcons1().get(position));
        }
