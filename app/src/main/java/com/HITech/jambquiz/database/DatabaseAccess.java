package com.HITech.jambquiz.database;
...
import com.HITech.jambquiz.model.DailyModel;
import com.HITech.jambquiz.model.HistoryModel;
import com.HITech.jambquiz.model.ProgressModel;
...
public class DatabaseAccess {
    ...
    public List<DailyModel> getDailyQuizFractionData() {
        Cursor cur = db.rawQuery("select * from economicsData  ORDER BY RANDOM() LIMIT  " + 1 + ";", null);
        List<DailyModel> item_data = new ArrayList<>();
        if (cur.getCount() != 0) {
            if (cur.moveToFirst()) {
                do {
    ...
    public List<QuizModel> getFractionQuizData(int main_id, int ref_id) {
        Cursor cur = db.rawQuery("select * from economicsData where " + MAIN_ID + " =" + main_id + " and " + REF_ID + " =" + ref_id + " ORDER BY RANDOM() LIMIT  " + Constant.DEFAULT_QUESTION_SIZE + ";", null);
        List<QuizModel> item_data = new ArrayList<>();
        if (cur.getCount() != 0) {
            if (cur.moveToFirst()) {
                do {
    ...
    public List<QuizModel> getWorksheetFractionData(int main_id, int ref_id, int limit) {
        Cursor cur = db.rawQuery("select * from economicsData where " + MAIN_ID + " =" + main_id + " and " + REF_ID + " =" + ref_id + " ORDER BY RANDOM() LIMIT  " + limit + ";", null);
        List<QuizModel> item_data = new ArrayList<>();
        if (cur.getCount() != 0) {
            if (cur.moveToFirst()) {
                do {
